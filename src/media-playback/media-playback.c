...
void media_playback_set_speed(media_playback_t *mp, int speed)
{
	if (!mp || mp->is_cached)
		return;

	mp_media_set_speed(&mp->media, speed);
}

int media_playback_get_speed(media_playback_t *mp)
{
	if (!mp || mp->is_cached)
		return 100;

	return mp_media_get_speed(&mp->media);
}

void media_playback_step_frames(media_playback_t *mp, int frames)
{
	if (!mp || mp->is_cached)
		return;

	mp_media_step_frames(&mp->media, frames);
}
