...
* **Take** and **Cancel**, which are there only while a shot is waiting and take no room at all the rest of the time. Confirm framing itself is in the dock's right-click menu, since it is set once rather than ridden.
...
Right-click the dock for **Full** or **Minimal**, and for **Confirm framing**. Confirm is a setting on the source the dock is pointed at, so the menu reads it back from that source every time it is opened, and it is the same setting the source properties carry. Minimal drops the picture and keeps everything else — the source list, the zoom, the pad, the presets and the speed — for an operator who frames from presets and the pad and would rather have the room. With the picture gone the pad is what the panel is, so it is let grow further than it does beside one. Nothing is rendered while it is minimal, so the dock costs only what its widgets cost. The choice is saved with your OBS user settings rather than with the scene collection, since it is how you like to work rather than anything about the show.
