...
* **Volume**, from silent to 100% of the audio in the files themselves. It is set before the playlist reaches the mixer, so the fader, mute, monitoring and audio filters all still apply on top of it. It applies live, to the file that is playing as well as to the ones after it, so a playlist of clips recorded far louder than the rest of the show can be turned down without touching the files.
